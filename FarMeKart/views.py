...
from FarMeKart.forms import UsregFo,ChpwdForm,UpdPfle,Vegfr,UpdVgtab
from django.contrib.auth.decorators import login_required
from farmer import settings
from django.core.mail import send_mail
from django.contrib.auth.models import User
from FarMeKart.models import Vegpro,UserPro,Cart,Myorders
...
def cartdetails(re):
	c=Cart.objects.filter(user_id=re.user.id)
	d=Vegpro.objects.all()
	sum=0
	count=0
	for k in c:
		count=count+1
		sum=sum+k.price
	return render(re,'html/usercart.html',{'sum':sum,'count':count,'data':d,'cart':c})

def order(re):
	return render(re,'html/myorders.html')
